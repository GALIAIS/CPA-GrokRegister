...
[![Grok Register — CLI registration automation toolkit](assets/banner.png)](https://github.com/AaronL725/grok-register)

Grok Register 是一个面向自动化流程研究、测试环境验证和个人学习的 Python 自动化注册工具 — 支持 CLI、临时邮箱、浏览器流程控制、账号输出和 grok2api token 池写入。
...
仅 CLI。注册流程仍会打开 Chromium/Chrome 浏览器页面（headless 可配）。
...
```bash
python grok_register_ttk.py start          # 使用 config register_count
python grok_register_ttk.py start 50       # 覆盖数量
python grok_register_ttk.py loop           # 不限量持续注册
```

Windows 也可双击：

- `start_register.bat` / `start_register.bat 50`
- `start_register_forever.bat`
...
注册页、Turnstile、验证码提交和 SSO cookie 获取依赖真实浏览器环境（可用 headless）。
...
直接读取 `config.json` 的 `register_count`；也可用 `start N` 覆盖。不限量用 `loop`。

## 目录结构

```text
.
├── grok_register_ttk.py   # 主程序（CLI 注册）
├── cpa_export.py          # CPA xAI 导出入口
├── cpa_xai/               # CPA mint / OAuth / schema
├── cf_mail_debug.py       # Cloudflare 邮箱调试工具
├── config.example.json    # 配置示例
├── requirements.txt       # Python 依赖
├── start_register.bat     # Windows 一键启动
├── start_register_forever.bat
└── README.md
```
