...
def export_cpa_xai_for_account(
    ...
    *,
    ...
    prefer_protocol = bool(cfg.get("cpa_protocol_mint", True)) and bool(sso_val)
    log(
        f"[cpa] mint OIDC for {email} -> {out_dir} proxy={proxy or '(none)'} "
        f"mode={'protocol' if prefer_protocol else 'browser'} "
    ...
    if prefer_protocol:
        try:
            from cpa_xai.protocol_mint import mint_and_export_protocol  # type: ignore

            result = mint_and_export_protocol(
                email=email,
                sso=sso_val,
                password=password,
            ...
            _log(f"protocol mint error, fallback browser: {e}")
            result = {"ok": False, "error": str(e)}

    if not result.get("ok"):
        if prefer_protocol:
            _log("falling back to browser device mint...")
    ...
    # 统一走 publish：本地 CLIProxyAPI auths 热加载 + 可选远程 SCP
    if result.get("ok") and result.get("path"):
        try:
            from grok_register_ttk import publish_auth_to_cpa

            pub = publish_auth_to_cpa(result["path"], log_callback=log)
            if pub.get("hotload_path"):
                result["cpa_path"] = pub["hotload_path"]
            if pub.get("hotload_error"):
                result["cpa_copy_error"] = pub["hotload_error"]
            if pub.get("remote_error"):
                result["upload_error"] = pub["remote_error"]
        except Exception as e:  # noqa: BLE001
            # 回退旧逻辑：直接 copy hotload
            if cfg.get("cpa_copy_to_hotload", True) and cpa_dir:
                try:
                    ...
                    log(f"[cpa] hotload copy failed: {e2}")
                    result["cpa_copy_error"] = str(e2)
            else:
                log(f"[cpa] publish failed: {e}")
