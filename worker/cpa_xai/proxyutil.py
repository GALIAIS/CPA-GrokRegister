...
def normalize_proxy_for_http(proxy: str) -> str:
    """Normalize proxy URL for curl_cffi / requests.

    Prefer socks5h (remote DNS) so SOCKS exits resolve hostnames correctly
    and avoid curl error 97 on some MicroWARP / WARP SOCKS stacks.
    """
    p = (proxy or "").strip()
    if not p:
        return ""
    ...
    if low.startswith("socks4://") and not low.startswith("socks4a://"):
...
def resolve_proxy(explicit: str | None = None) -> str:
    for cand in (
        ...
        if cand:
            return normalize_proxy_for_http(cand)
    return ""
...
def proxy_for_chromium(proxy: str) -> str:
    ...
    if not p:
        return ""
    # Chromium uses socks5 (not socks5h scheme name)
